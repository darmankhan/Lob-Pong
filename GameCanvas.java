...
public class GameCanvas extends JComponent implements KeyListener {
	...
	protected int rectX, rectY, velocityX = 200, velocityY = 200, ballSize = 20, rectWidth = 100, rectHeight = 30, lives = 3, score = 0, actualTime = 0,
			gameTime = 0, levelTime = 10, level = 1, highScore = Highscore.read(), adjustToMiddle = 100;
	protected String rectDirection;
	protected double ballX, ballY, dirX = -1, dirY = 1, theta = Math.toRadians(50), time = 0.3, increaseBallX, increaseBallY, timeX, 
			timeY;
	...
	public void updateTimer(Graphics g) { //draws timer
		...
		if (actualTime < levelTime) { //timer is only drawn if it's lesser than time of level
			...
			levelTime = 5*level; //levelTime increases by 10 after every level
